    ...
    <div className="flex justify-center w-full py-6 px-4">
      <div className="flex items-center justify-between px-6 py-3 bg-white rounded-full shadow-lg w-full max-w-3xl relative z-10">
        <div className="flex items-center">
          ...
          >
          <Link href="/">
           <Image src="logo1.png" alt="main logo" width={100} height={100} />
           </Link>
          </motion.div>
        </div>
          ...
          <nav className="hidden md:flex items-center space-x-8">
            {NavItems.map((item) => (
              ...
              >
                <a href={item.link} className="text-sm text-gray-900 hover:text-gray-600 transition-colors font-medium">
                  {item.name}
                </a>
              </motion.div>
            ))}
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="flex flex-col space-y-6">
              {["Home", "About", "Terms", "Contact"].map((item, i) => (
                <motion.div
                  key={item}
                  initial={{ opacity: 0, x: 20 }}
                  animate={{ opacity: 1, x: 0 }}
                  ...
                  exit={{ opacity: 0, x: 20 }}
                >
                  <a href={item} className="text-base text-gray-900 font-medium" onClick={toggleMenu}>
                    {item}  
                  </a>
                </motion.div>
              ))}
